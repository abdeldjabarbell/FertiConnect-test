...
async function signUp() {
    ...
    try {
        ...
        await setDoc(doc(db, "medecin", userCredential.user.uid), {
            nom: registerNom,
...
loginForm.addEventListener("submit", async (e) => {
    ...
    const loginEmail = document.getElementById("registerEmail_medecin").value;

    // Vérification si le email existe dans la collection "users"
    const q = query(collection(db, "medecin"), where("email", "==", loginEmail));
    getDocs(q)
        .then((querySnapshot) => {
